...
public class FSOnlineGame extends FrameState {
    ...
    public FSOnlineGame(MainFrame frame, int paneWidth, int paneHeight) throws ScriptException, NoSuchMethodException {
        ...
        _gameManager.setSessionManager(_sessionManager);
        _gameManager.setOnlineout(_onlineOut);
        ...
        if (player.isLocal()) {
            ...
            try {
                ...
                do {
                    ...
                    if (input.equals("moveCard")) {
                        break;
                    }
                    
                    int cardID = Integer.parseInt(_onlineIn.readLine());
                    int deckID = Integer.parseInt(_onlineIn.readLine());
                    int deckPos = Integer.parseInt(_onlineIn.readLine());
                    
                    _gameManager.uncheckedMoveCard(_gameManager.getCard(cardID), _gameManager.getDeck(deckID), deckPos);

                } while(true);
                
                input = _onlineIn.readLine();
                if (input.equals("endTurn")) {
                    endTurnAction.run();
                }                
